...
* DEFININING GAS PARAMETERS FOR SCENARIO
        ...
        nonPowerDem(co,year,month,scen) = cons_ST(co, year, month)*(1-PS_ST(co, year));
...
* DEFININING EL PARAMETERS FOR SCENARIO
        ...
        demand(co,t,year,scen)              = demand_upload(co,t,year,"ST_up");
        cap_existing(i,n,year,scen)         = cap_existing_upload(i,n,year,"ST_up");
        fc(i,co,year,scen)                  = fc_upload(i,co,year,"ST_up") ;
        co2_price(scen,year)                = co2_price_upload("ST_up", year);
        vc_f(i,co,year,scen)                = (fc_upload(i,co,year,"ST_up")+ carb_cont(i)*co2_price_upload('ST_up',year))/ eta_f(i,co,year);
        vc_m(i,co,year,scen)                = (fc_upload(i,co,year,"ST_up")+ carb_cont(i)*co2_price_upload('ST_up',year))/ eta_m(i,co,year);
...
$include IntEG_v20_S3(ECIU_ST)_solve.gms

$include IntEG_v20_resprocess.gms

        put_utility 'gdxout' / 'output_v20_ST\v20_STOCH_Scen_' UncPar.tl:0;
        execute_unload  ;
...
$Load  geninv_DET_ST  = CAP_new.l
$gdxin

CAP_new.fx(i,co,year)   = geninv_DET_ST(i,co,year);
...
$include IntEG_v20_resprocess.gms
        ...
        put_utility 'gdxout' / 'output_v20_ST\v20_STOCH_FINV_Scen_' UncPar.tl:0;
        execute_unload  ;
